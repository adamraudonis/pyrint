...
for c in $CORPORA; do
  ...
  python3 "$ROOT/harness/strip_footer.py" "$ROOT/harness/results/$c.hook.out" > /tmp/p_gt_b 2>/dev/null
  python3 "$ROOT/harness/strip_footer.py" "$ROOT/harness/results/$c.hook.ours" > /tmp/p_ours_b 2>/dev/null
