...
prylint is single-threaded in the inference/checking phase by design: the
...
exactly. Parallelizing across files would change cache-warming order and
reintroduce divergences — so the single-core speed (still 15–2300× pylint) is
the *correct* speed. File parsing is already parallelized.
